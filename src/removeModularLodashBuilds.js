...
module.exports = async function removeModularLodashBuilds() {
  ...
  const lodashModularDependencies = Object.keys(dependencies || {}).filter(
    isLodashModularPackage
  )
  const lodashModularDevDependencies = Object.keys(
    devDependencies || {}
  ).filter(isLodashModularPackage)
  const lodashModularPeerDependencies = Object.keys(
    peerDependencies || {}
  ).filter(isLodashModularPackage)
  const lodashModularOptionalDependencies = Object.keys(
    optionalDependencies || {}
  ).filter(isLodashModularPackage)
  ...
  const isDev =
    lodashModularDevDependencies.length &&
  ...
  if (await isFile(path.resolve(root, 'yarn.lock'))) {
    ...
    await spawn(
      'yarn',
      isDev ? ['add', '--dev', 'lodash'] : ['add', 'lodash'],
      spawnOpts
    )
    ...
    await spawn(
      'npm',
      ['remove', '--save', ...lodashModularPackages],
      spawnOpts
    )
    await spawn(
      'npm',
      ['add', isDev ? '--save-dev' : '--save', 'lodash'],
      spawnOpts
    )
  ...
  if (babelrcChanged)
    await fs.writeFile(babelrcFile, JSON5.stringify(babelrc, null, 2))
