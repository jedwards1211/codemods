...
const esModules = readdirSync(path.dirname(require.resolve('lodash-es/package.json')))

const esModulesMap = new Map(esModules.map(file => {
...
module.exports = function switchToLodashES(fileInfo, api) {
  ...
  if (root
    ...
    .find(j.ImportDefaultSpecifier).size()
  ) {
    throw new Error('WARNING: default imports not supported')
  }
  root.find(j.ImportDeclaration, {source: {value: value => subpkgRx.test(value)}})
    .forEach(path => {
  ...
  root.find(j.ImportDeclaration, {source: {value: 'lodash'}})
    .replaceWith(path =>
      path.value.specifiers.map(specifier => (
        statement([`import ${specifier.local.name} from 'lodash-es/${specifier.imported.name}'\n`])
      ))
    )
