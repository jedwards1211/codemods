...
function addHasManyAssociation({
  root,
  position,
  target,
  primaryKeyType,
  ...
  const newProperties = [
    ...
    classProperty(`set${upperFirst(asPlural)}`, 'HasManySetMany', [
      target,
      primaryKeyType,
    ]),
    classProperty(`add${upperFirst(asPlural)}`, 'HasManyAddMany', [
      target,
      primaryKeyType,
    ]),
    classProperty(`add${upperFirst(asSingular)}`, 'HasManyAddOne', [
      target,
      primaryKeyType,
    ]),
    classProperty(`create${upperFirst(asSingular)}`, 'HasManyCreateOne', [
      `${target}InitAttributes`,
      target,
    ]),
    classProperty(`remove${upperFirst(asSingular)}`, 'HasManyRemoveOne', [
      target,
      primaryKeyType,
    ]),
    classProperty(`remove${upperFirst(asPlural)}`, 'HasManyRemoveMany', [
      target,
      primaryKeyType,
    ]),
    classProperty(`has${upperFirst(asSingular)}`, 'HasManyHasOne', [
      target,
      primaryKeyType,
    ]),
    classProperty(`has${upperFirst(asPlural)}`, 'HasManyHasMany', [
      target,
      primaryKeyType,
    ]),
  ...
  initAssociationsBody.push(
    j.expressionStatement(
      j.assignmentExpression(
        ...
        j.callExpression(
          j.memberExpression(j.thisExpression(), j.identifier('hasMany')),
          [
            j.identifier(target),
            j.objectExpression([
