...
module.exports = function addEslintDisableLine({eslintJsonOutput, ruleIds, dryRun}) {
  ...
  for (let {filePath, messages} of eslintJsonOutput) {
    ...
    messages = messages.filter(({ruleId, severity}) =>
      severity >= 2 && ruleIds.has(ruleId)
    )
    if (!messages.length) continue
    const lines = fs.readFileSync(filePath, 'utf8').split(/\r\n?|\n/mg)
    let prevLine
    for (const {line, ruleId} of messages) {
      lines[line - 1] += `${line === prevLine ? ',' : ' // eslint-disable-line'} ${ruleId}`
      prevLine = line
    }
