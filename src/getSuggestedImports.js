...
const getSuggestedImportsForDir = memoize(async function (dir) {
  ...
  const parentDirSuggestions = parentDir && parentDir !== dir
  ...
  if (suggestedImports) {
    for (let i = suggestedImports.length - 1; i >= 0; i--) {
      ...
      if (typeof statement === 'string') {
        let parsed = []
        try {
          parsed = j(statement).find(j.ImportDeclaration).nodes()
        } catch (error) {
          console.error(`Invalid import statement in: ${statement}`, error.stack) // eslint-disable-line no-console
        }
