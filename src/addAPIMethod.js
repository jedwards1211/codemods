...
module.exports = function addAPIMethod(root, position, {name, options, result, returnType, appContextType, file}) {
...
`]),
    !returnType && statement([`export type ${upper}Result = {
  ${result || ''}
}
...
`]),
    statement([`export async function ${lower}(options: ${upper}Options): Promise<${returnType || `${upper}Result`}> {
