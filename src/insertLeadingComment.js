...
module.exports = function insertLeadingComment(root, value) {
  ...
  if (program.comments && program.comments.findIndex(comment => comment.value.trim() === value.trim()) < 0) {
    program.comments.push(commentLine)
  }
  if (program.body.length) {
    const firstStatement = program.body[0]
    if (firstStatement.comments && firstStatement.comments.findIndex(comment => comment.value.trim() === value.trim()) < 0) {
