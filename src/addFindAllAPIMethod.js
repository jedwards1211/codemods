...
module.exports = function addFindAllAPIMethod(
  root,
  position,
  ...
  insertProgramStatement(
    ...
    position,
    statement([
      `export type FindAll${plural}Options = {
...
`,
    ...
    statement([
      `export const FindAll${plural}OptionsType = (${reify}: ${Type}<FindAll${plural}Options>)

`,
    ]),
    statement([
      `export async function assertCanFindAll${plural}(options: FindAll${plural}Options): Promise<void> {
...
`,
    ...
    statement([
      `export async function getFindAll${plural}Query(options: FindAll${plural}Options): Promise<${FindOptions}<${modelName}>> {
  const { apiContext } = options
...
`,
    ...
    statement([
      `export async function findAll${plural}(options: FindAll${plural}Options): Promise<Array<${modelName}>> {
