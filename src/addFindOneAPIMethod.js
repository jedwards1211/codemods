...
module.exports = function addFindOneAPIMethod(
  root,
  position,
  { modelName, appContextType, file }
) {
  insertLeadingComment(root, ' @flow-runtime enable')
  const { FindOptions } = addImports(
    root,
    statement`import type {FindOptions} from 'sequelize'`
  )
  modelName = addImports(
    root,
    statement([`import ${modelName} from '../models/${modelName}'`])
  )[modelName]
  ...
  insertProgramStatement(
    ...
    position,
    statement([
      `export type FindOne${modelName}Options = {
...
`,
    ...
    statement([
      `export const FindOne${modelName}OptionsType = (${reify}: ${Type}<FindOne${modelName}Options>)

`,
    ]),
    statement([
      `export async function assertCanFindOne${modelName}(options: FindOne${modelName}Options): Promise<void> {
...
`,
    ...
    statement([
      `export async function getFindOne${modelName}Query(options: FindOne${modelName}Options): Promise<${FindOptions}<${modelName}>> {
  const { apiContext } = options
...
`,
    ...
    statement([
      `export async function findOne${modelName}(options: FindOne${modelName}Options): Promise<?${modelName}> {
