...
module.exports = function insertProgramStatement(
  root,
  position,
  ...statements
) {
  ...
  const program = root
    ...
    .at(0)
    .nodes()[0]
  let index = program.body.findIndex(node => node.end >= position)
  ...
  if (index === 0) {
    if (program.body.length && program.body[0].comments) {
      ...
      statement.comments = [...program.comments, ...(statement.comments || [])]
      program.comments = []
    }
