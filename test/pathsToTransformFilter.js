...
function nthIndexOf(str, n, searchValue) {
  let fromIndex = 0
  while (n-- >= 0 && fromIndex >= 0) {
    fromIndex = str.indexOf(searchValue, fromIndex)
  }
  return fromIndex
}
...
`
  ...
  it(`if any path are fully contained in the range, returns only the outermost ones`, function () {
    let nodes = lambdas.filter(pathsToTransformFilter(
      code.indexOf('toink'),
      code.indexOf('nelm')
    )).nodes()
 ...
 `() => {
    ...
    nodes = lambdas.filter(pathsToTransformFilter(
      code.indexOf('qlomb'),
      code.indexOf('flamb')
    )).nodes()
